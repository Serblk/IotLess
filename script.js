...
function loadPdf(idx){
  ...
  loadingMsg.className="viewer-loading";
  loadingMsg.innerHTML='<div class="spinner"></div><span>Загрузка PDF…</span>';
  container.appendChild(loadingMsg);

  if(location.protocol==="file:"){
    container.innerHTML='<p style="padding:40px;text-align:center;color:var(--text2)">Просмотр PDF недоступен при открытии через file://. <a href="'+item.file+'" target="_blank" style="color:var(--accent)">Открыть PDF в новой вкладке</a></p>';
    return;
  }
  pdfjsLib.getDocument({url: item.file}).promise.then(doc=>{
    container.innerHTML="";
    ...
    container.innerHTML='<p style="padding:40px;text-align:center;color:var(--text2)">Ошибка загрузки PDF: '+err.message+'</p>';
  });
