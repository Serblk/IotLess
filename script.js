...
function loadPdf(idx){
  ...
  pdfjsLib.getDocument({url:item.file,disableRange:true,disableStream:true}).promise.then(doc=>{
    currentPdfDoc=doc;
    isViewerLoading=false;
    ctx.style.minHeight="auto";
    const pagesContainer=document.getElementById("viewerPages");
    ...
    const renderPage=function(pageNum){
      doc.getPage(pageNum).then(page=>{
        ...
        page.render({canvasContext:canvas.getContext("2d"),viewport:vp}).promise.then(()=>{
          currentPdfPages.push(pageNum);
          rendered++;
          loading.innerHTML='<div class="spinner"></div><span>Загрузка PDF\u2026 '+rendered+'/'+total+'</span>';
          if(rendered===total)loading.style.display="none";
          updateViewerPosition();
        });
      });
    };
    ...
    loading.innerHTML='<p>Ошибка загрузки PDF: '+err.message+'</p><button class="btn btn-sm" onclick="loadPdf('+idx+')" style="margin-top:12px">Повторить</button>';
    isViewerLoading=false;
  });
  updateViewerPosition();
  renderViewerSidebar();
...
function updateViewerPosition(){
  ...
  if(currentPdfDoc&&currentPdfPages.length>0){
    document.getElementById("viewerPosition").textContent="\u0421\u0442\u0440. "+currentPdfPages.length+" \u0438\u0437 "+currentPdfDoc.numPages;
  } else {
